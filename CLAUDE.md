...
Sistema de administracion de feria (Fair Sys) desarrollado en Angular 21 con Angular Material.
...
│   └── reports/
│       └── product-visits/
│           # Cada feature: components/ + [feature].ts
│
...
/admin/users                 -> AdminLayout > Admin > Usuarios
/admin/categories            -> AdminLayout > Admin > Categorias
/admin/products              -> AdminLayout > Admin > Productos
/admin/exhibitors            -> AdminLayout > Admin > Expositores
/admin/fair-editions         -> AdminLayout > Admin > Ediciones de Feria
/admin/meetings              -> AdminLayout > Admin > Reuniones
/reports/product-visits      -> AdminLayout > Reportes > Visitas por Producto
**                           -> redirect /auth/login
```
...
> Todos los modulos de negocio viven bajo **Admin**. Los que no existan aun deben crearse como mockups con data estatica.
...
### Dashboard
- Vista principal con KPIs: expositores activos, productos, reuniones, visitas.

### Admin
CRUDs del modulo (ver detalle de columnas en `.claude/docs/entities.md`):

| Ruta | Entidad | Tabla |
|---|---|---|
| `/admin/users` | `User` | `users` |
| `/admin/fair-editions` | `FairEdition` | `fair_editions` |
| `/admin/categories` | `Category` | `categories` |
| `/admin/exhibitors` | `Exhibitor` | `exhibitors` |
| `/admin/products` | `Product` | `products` |
| `/admin/meetings` | `Meeting` | `meetings` |

**Enums globales:** `State` (ACTIVE / INACTIVE) · `UserRole` (ADMIN / EXHIBITOR / VISITOR) · `MeetingState` (PENDING / CONFIRMED / CANCELED / COMPLETED)

### Reportes
- `/reports/product-visits` — tabla de visitas agrupadas por producto (`product_visits` + `products`).
...
Sistema de administracion de feria: gestiona ediciones, expositores, productos, imagenes, visitas y reuniones entre usuarios.
